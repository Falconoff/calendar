...
let showDateBlock = document.querySelector(".showDate"); // отобразим над календарём дату
...
let dates = document.querySelector(".calendar__dates");  // поле календаря для ячеек с датами
...
function showCurrentMonth() {
	...
	showSelectedMonth(yearToday, monthToday, dateToday); // отображаем текущий месяц в календаре
...
function showSelectedMonth(year, month, date) {
	...
	// let dates = document.querySelector(".calendar__dates");  объявлена выше
	let i=1;
	// ВЫВОДИМ ЯЧЕЙКУ предыдущего дня предыдущего месяца <div class="calendar__day disabled">дата</div>
	while (i <= firstDay) {
		let prevDate = (new Date(year, month, 1 - i)).getDate(); //получаем дату -1 дня
		let itemDay = document.createElement("div");
		itemDay.className = "calendar__day disabled";
	...
	// ВЫВОДИМ ЯЧЕЙКУ дня текущего месяца <div class="calendar__day">дата</div>
	let j=1;
	while (j <= numDays) {
...
if (myYear == yearToday && myMonth == monthToday && j == dateToday) {
	itemDay.className = "calendar__day today";
} else {
	itemDay.className = "calendar__day";
}
		// itemDay.className = "calendar__day";
	...
	// ВЫВОДИМ ЯЧЕЙКУ следущего дня следущего месяца <div class="calendar__day disabled">дата</div>
